...
ostream& operator <<(ostream& os, const big_integer& bi) {
    if (bi._digits.empty()) os << 0;
        ...
        for (long long i = static_cast<long long>(bi._digits.size()) - 2; i >= 0; --i) os << setw(9) << bi._digits[i];
        os.fill(old_fill);
    }
...
big_integer::big_integer(signed int i) {
    ...
    this->_digits.push_back(abs(i) % big_integer::BASE);
    i /= big_integer::BASE;
...
big_integer::big_integer(signed long l) {
    ...
    this->_digits.push_back(abs(l) % big_integer::BASE);
    l /= big_integer::BASE;
    if (l != 0) this->_digits.push_back(abs(l));
}
